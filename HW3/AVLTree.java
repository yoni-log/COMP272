...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
        ...
        // if the value is smaller than the node's value then the node is to the left
        if (value < node.value) {
            node.leftChild = deleteElement(value, node.leftChild);
        }
        // if the value is greater than the node's value then the node is to the right
        else if (node.value < value) {
            node.rightChild = deleteElement(value, node.rightChild);
        }
        else {
            // the node can be replaced by the right child if the left child is null
            if (node.leftChild == null) {
                return node.rightChild;
            }
            // the node can be replaced by the left child if the right child is null
            else if (node.rightChild == null) {
                return node.leftChild;
            }

            // smallest in the right subtree (inorder successor)
            Node temp = minValueNode(node.rightChild);
            // replace the current node with the successor
            node.value = temp.value;
            // keep recursing with the right subtree
            node.rightChild = deleteElement(temp.value, node.rightChild);
        }
        // update the height and balance of the node to check if unbalanced
