...
class myHashMap<K,V> {
    ...
    public V remove(K key) {
        // get the index of the key to be removed
        int index = getBucketIndex(key);

        // set our previous and current nodes
        HashNode<K, V> node = bucket.get(index);
        HashNode<K, V> prev = null;

        while (node != null) {
            // check if the current key matches the key to be removed
            if (node.key.equals(key)) {
                // remove the current key by linking the previous node with the next node, eliminating the current node
                if (prev != null) {
                    prev.next = node.next;
                }
                // special case if we are removing the head of the nodes
                else {
                    // set the next node as the head of the nodes
                    bucket.set(index, node.next);
                }
                // decrement the size after removal and return the value of the removed node
                size--;
                return node.value;
            }
            prev = node;
    ...
    public V replace(K key, V value) {
        ...
        while (node != null) {
            // check if the current key matches with the key to be replaced
            if (node.key.equals(key)) {
                // if they match, store the old value, replace the old value with the new value, and return the old value
    ...
    public boolean replace(K key, V oldVal, V newVal) {
        ...
        // check if the values can be replaced
        if ((currVal == null && oldVal == null) || (currVal != null && currVal.equals(oldVal))) {
            // call the replace method to replace the values
            replace(key, newVal);
