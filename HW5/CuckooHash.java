...
public class CuckooHash<K, V> {
	...
	public void put(K key, V value) {
		int attempts = 0;
		int maxAttempts = CAPACITY * 2;
		Bucket<K, V> newBucket = new Bucket<>(key, value);

		while (attempts < maxAttempts) {
			int pos1 = hash1(newBucket.getBucKey());

			// Check for duplicate key with same value at pos1
			if (table[pos1] != null && table[pos1].getBucKey().equals(key)) {
				if (table[pos1].getValue().equals(value)) return; // Duplicate, no insertion
				table[pos1].value = value; // Update value if key is found with a different value
				return;
			}
			...
			Bucket<K, V> evictedBucket = table[pos1];
			table[pos1] = newBucket;

			int pos2 = hash2(evictedBucket.getBucKey());

			// Check for duplicate key with same value at pos2
			if (table[pos2] != null && table[pos2].getBucKey().equals(evictedBucket.getBucKey())) {
				if (table[pos2].getValue().equals(value)) return;
				table[pos2].value = value;
				return;
			}

			if (table[pos2] == null) {
				table[pos2] = evictedBucket;
				return;
			}

			newBucket = table[pos2];
			table[pos2] = evictedBucket;
			attempts++;
		}

		// If we reach maxAttempts, rehash the table
		rehash();
		put(key, value); // Retry after rehashing
	}
	...
	public void rehash() {
		Bucket<K, V>[] oldTable = table.clone();
