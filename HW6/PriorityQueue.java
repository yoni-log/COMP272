...
class PriorityQueue<E, P> {
    ...
    public Node add(E e, P priority) {
        ...
        // Add and reheapify by pulling the value up in the tree
        tree.add(node);
        pullUp(node.idx);
        return node;
    }
    ...
    public boolean contains(E e) {
        // We can iterate through all the values in the tree since it is a heap
        for (Node node : tree) {
            // Edge case if they are both null
            if (node.value() == null && e == null) {
                return true;
            }
            // Check if the value exists and compare the objects to check if they are the same
            if (node.value() != null && node.value().equals(e)) {
                return true;
            }
        }
        // The tree does not contain the value
        return false;
    }
