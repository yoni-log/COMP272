...
public class ProblemSolutions {
  ...
  public static int lastBoulder(int[] boulders) {
      // Start in reverse order, so the max element is given at the top
      PriorityQueue<Integer> queue = new PriorityQueue<>(Collections.reverseOrder());
      for (int boulder : boulders) queue.add(boulder);
      while (queue.size() > 1) {
          int b1 = queue.poll();
          int b2 = queue.poll();
          // Condition #1: x is destroyed (b2 in this case), leading to the y boulder being weakened
          if (b1 != b2) {
              queue.add(b1 - b2);
          }
          // Condition #2: We don't need the b1==b2 condition since both are destroyed
      }
    ...
    public static ArrayList<String> showDuplicates(ArrayList<String> input) {
        ...
        for (String str : input) {
            // We can check if there are duplicates if we can't add to the set, also checking that the list
            // of duplicates does not contain a known duplicate value
    ...
    public static ArrayList<String> pair(int[] input, int k) {
        ...
        // We can solve this in O(n) since we can find the pair by reversing the process
        for (Integer num : input) {
            // Check for each number if the set contains a corresponding number to create a pairing
        ...
        // Sort and return as an array list
        ArrayList<String> arr = new ArrayList<>(pairs);
