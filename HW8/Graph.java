...
public class Graph {
  ...
  public int findRoot() {
    int[] verticies = new int[numVertices]; // array to track vertices
    for (int i = 0; i < numVertices; i++) {
      for (int index : adjListArr[i]) {
        verticies[index]++;
      }
    }

    int root = -1; // initialize root as -1 (not found)
    for (int i = 0; i < numVertices; i++) {
      if (verticies[i] == 0) {
        if (root != -1) {
          return -1;
    ...
    // return the value of the root vertex, or -1 if no single root exists
