...
class ProblemSolutions {
    ...
    public boolean canFinish(int numExams, int[][] prerequisites) {
        ...
        boolean[] visited = new boolean[numNodes]; // tracks nodes that have been fully processed
        boolean[] recStack = new boolean[numNodes]; // tracks nodes in the current recursive call stack
        for (int i = 0; i < numNodes; i++) { // iterate through all nodes
            if (checkIfCyclic(i, adj, visited, recStack)) return false; // detect cycle in the graph
        }
        return true;
    }

    private boolean checkIfCyclic(int node, ArrayList<Integer>[] adj, boolean[] visited, boolean[] recStack) {
        if (recStack[node]) return true;
        if (visited[node]) return false;
        visited[node] = true;
        recStack[node] = true;
        // iterate through all nodes and recursively check for cycles
        for (int adjVals : adj[node]) {
            if (checkIfCyclic(adjVals, adj, visited, recStack)) return true;
        }
        recStack[node] = false; // remove the node
        return false;
    }
    ...
    public int numGroups(int[][] adjMatrix) {
        ...
        int groups = 0; // count of connected groups
        boolean[] visited = new boolean[numNodes]; // tracks visited nodes

        // iterate through all nodes
        for (int node = 0; node < numNodes; node++) {
            if (!visited[node]) {
                // start a dfs for each unvisited node and increment the groups
    ...
    private void dfs(int currNode, Map<Integer, List<Integer>> graph, boolean[] visited) {
        ...
        // iterate through the node's neighbors if it has any
